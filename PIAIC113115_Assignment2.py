...
def function8():
    ...
    x = np.arange(10, dtype=np.uint32).reshape(2,5); x.fill(6) #write your code here
...
def function9():
    ...
    a = np.arange(1, 101);a = a[a%2==0 ] # write your code here
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = ans=np.hstack(((np.repeat(arr, 3)),(np.hstack((arr,arr,arr))))) #write your code here 
...
def function14():
    ...
    ans = np.split(arr,[3,6,9,12,15,18,21]) #write your code here 
...
def function19():
    ...
    output = np.arange(1,26).reshape(5,5);w=x;w=np.transpose(w);b=5;a=(x*w)+b   # Write your code here
    return output
    #expected output
    """
